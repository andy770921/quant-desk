...
/**
 * Improved vol-target: a 200-SMA trend gate throttles the target vol off-trend
 * (cuts bear-market exposure), and idle capital sits in a trending Treasury.
 */
export const volTargetSpyPlus: StrategyDefinition = {
  id: 'vol-target-spy-plus',
  name: '波動度目標（改良版）',
  shortName: '波動度目標+',
  category: 'volatility',
  description:
    '加上 200 日均線趨勢閘門：趨勢向上目標波動 15%，跌破則降為 5%；閒置資金改持趨勢公債。',
  longDescription:
    '原版波動度目標的改良。單純波動度目標在空頭仍可能滿倉，因此加上 200 日均線趨勢閘門：趨勢向上時目標波動 15%，' +
    '跌破均線時把目標波動降到 5%（大幅縮減曝險）；未投入的資金不再閒置於現金，而是配置到趨勢向上的中期公債，提升閒置資金報酬，' +
    '兼顧降低回撤與提升 Sharpe。',
  rules: [
    '目標波動：標普 500 站上 200 日均線 → 15%；跌破 → 5%。',
    '股票曝險 = clamp(目標波動 ÷ 20 日波動度, 0, 2)。',
