...
export const nasdaq3x20dma: StrategyDefinition = {
  ...
  longDescription:
    '這是平台的旗艦策略：用 3 倍槓桿放大那斯達克 100 指數的報酬，並用最簡單的 20 日移動平均線作為進出場濾網。' +
    '當指數收盤價高於 20 日均線代表短期動能向上，便以 3 倍曝險參與上漲；一旦跌破 20 日均線就立刻全數轉為現金，' +
    '避免槓桿在下跌與盤整時造成的價值耗損 (volatility decay)。槓桿報酬以指數每日報酬 ×3、並扣除借貸成本與費用模擬，' +
    '因此可一路回測到 1990 年（早於 TQQQ 於 2010 年問世）。',
  rules: [
    '訊號標的：那斯達克 100 指數（不融資，槓桿曝險以買入 TQQQ 這檔 3 倍 ETF 達成）。',
    '進場：指數收盤價 > 20 日簡單移動平均線 → 100% 資金買入 TQQQ（3 倍那斯達克）。',
    '出場：指數收盤價 < 20 日均線 → 全數賣出轉為現金（賺取國庫券利息）。',
  ...
  signalFormula: [
    'price = level(NASDAQ)        // 用那斯達克100 指數判斷訊號',
    'ma20  = sma(NASDAQ, 20)',
    '',
    'if price > ma20:',
